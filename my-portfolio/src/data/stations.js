export const STATIONS = [
  {
    idx: 0,
    node: 'VI', name: 'FIRST Tech Challenge', elev: '6,643 ft',
    stationLabel: 'Station VI · The Summit',
    title: 'FIRST', titleEm: 'Tech Challenge',
    sub: 'Mentor · Grundy Center, IA · 2023 — Present',
    highlights: [
      { k: 'Students', v: null,       vEm: '30+',      vSuffix: null },
      { k: 'Teams',    v: '3 squads', vEm: null,       vSuffix: null },
      { k: 'Focus',    v: 'Mech · ',  vEm: 'Java',     vSuffix: null },
      { k: 'Seasons',  v: null,       vEm: '3',        vSuffix: ' years' },
    ],
    sectionHead: 'Notable moments',
    moments: [
      'Mentoring FTC Teams 13186, 14375, and 10069 in mechanical design, Java robot programming, and match strategy.',
      'Helping students go from rough prototype to competition-ready robot — debugging, documentation, and engineering communication included.',
      'Teaching students to think like engineers: identify root causes, communicate clearly, and iterate fast under pressure.',
      'Giving back to a program that builds real engineering skills through hands-on collaboration and competition.',
    ],
    tags: ['30+ students','Java robotics','Mechanical design','Match strategy'],
  },
  {
    idx: 1,
    node: 'V', name: 'The Kit', elev: '5,540 ft',
    stationLabel: 'Station V · The Cache',
    title: 'The', titleEm: 'kit',
    sub: 'Tools used in earnest — not just listed',
    skillGroups: [
      { h: 'Programming & Development', tags: ['Python','Java','Spring Boot','PowerShell','Git','LangChain','OpenCV'] },
      { h: 'Systems & Infrastructure',  tags: ['Windows Server 2025','Linux','VMware','Hyper-V','Active Directory','Group Policy','Exchange'] },
      { h: 'Security & Networking',     tags: ['Cryptography','Server Hardening','Network Config','Auth Systems','Pen Testing'] },
      { h: 'Embedded & Hardware',       tags: ['Raspberry Pi','RISC-V','Assembly','Environmental Sensors'] },
      { h: 'Homelab & Self-Hosting',    tags: ['NAS Storage','Pi-hole','DNS','Local AI','Ollama','Self-Hosted Services','Monitoring'] },
    ],
  },
  {
    idx: 2,
    node: 'IV', name: 'Side Projects', elev: '4,910 ft',
    stationLabel: 'Station IV · The Atelier',
    title: 'Five', titleEm: 'side builds',
    sub: 'Personal projects · 2024 — 2025',
    projects: [
      {
        t: 'RISC-V Processor', st: 'Architecture · 2024',
        d: 'A hardware-scheduled five-stage pipelined RISC-V processor built at the assembly level. Demonstrates CPU architecture, control signals, datapath design, memory access, and instruction-level debugging.',
        stack: ['RISC-V','Assembly','CPU Architecture','Datapath Design','Digital Logic'],
      },
      {
        t: 'KV Server', st: 'Systems · 2025',
        d: 'A multithreaded key-value storage server handling concurrent client requests with hash table storage, read/write locking, worker threads, and TTL-based expiration with automatic cleanup.',
        stack: ['C','Concurrency','R/W Locks','Hash Tables','TTL Expiration'],
      },
      {
        t: 'Nerd Market', st: 'Full-stack · 2025',
        d: 'A stock-ticker-style price tracker for Pokémon, Magic: The Gathering, and other TCG markets. Treats collectible cards like financial assets — tracking price movement, set metadata, and market trends.',
        stack: ['Java','Spring Boot','MariaDB','OpenCV','TCGPlayer API'],
      },
      {
        t: 'Outpost', st: 'Hardware · 2025',
        d: 'A portable offline survival computer on a Raspberry Pi 5 — Wikipedia, survival manuals, maps, environmental sensors, and a local Ollama Phi-3 model. Zero internet dependency by design.',
        stack: ['Raspberry Pi 5','Ollama','Phi-3','Linux','Sensors'],
      },
      {
        t: 'HelpLlama', st: 'ML · 2025',
        d: 'A Python helpdesk chatbot that parses DOCX, PDF, and TXT documentation and ticket history to answer IT questions in context. Built around a real pain point: the knowledge exists — nobody can find it.',
        stack: ['Python','LangChain','RAG','Local LLM'],
      },
    ],
  },
  {
    idx: 3,
    node: 'III', name: 'Cambridge Investments', elev: '3,840 ft',
    stationLabel: 'Station III · The Highland',
    title: 'Cambridge', titleEm: 'Investments',
    sub: 'IT Infrastructure Engineer · HCI Team · 2024 — 2025',
    highlights: [
      { k: 'Stint',    v: null,           vEm: '2024–25', vSuffix: null },
      { k: 'Stack',    v: 'Win ',         vEm: '2025',    vSuffix: null },
      { k: 'Exchange', v: null,           vEm: '99.9%',   vSuffix: ' up' },
      { k: 'Location', v: 'Fairfield, ',  vEm: 'IA',      vSuffix: null },
    ],
    sectionHead: 'Notable moments',
    moments: [
      'Configured and deployed Windows Server 2025 in VMware to support Active Directory, file sharing, and enterprise directory services.',
      'Built an isolated AD sandbox using a domain clone, enabling safe product testing and change validation before production.',
      'Developed Group Policy hardening baselines that improved endpoint security posture and established consistent configuration standards across the environment.',
      'Managed Exchange settings, monitored infrastructure health, and participated in outage response and root cause analysis to support business continuity.',
    ],
    tags: ['Win Server 2025','VMware','Active Directory','Group Policy','Exchange'],
  },
  {
    idx: 4,
    node: 'II', name: 'ISU IT Solution Center', elev: '2,280 ft',
    stationLabel: 'Station II · The Crossing',
    title: 'ISU', titleEm: 'IT Solution Center',
    sub: 'Team Lead · Senior Technical Support · 2023 — Present',
    highlights: [
      { k: 'Role',    v: null,      vEm: 'Team Lead', vSuffix: null },
      { k: 'Since',   v: '2023 — ', vEm: 'now',       vSuffix: null },
      { k: 'Manages', v: null,      vEm: '10–15',     vSuffix: ' techs' },
      { k: 'Onboard', v: '6 wk → ', vEm: '2 wk',     vSuffix: null },
    ],
    sectionHead: 'Notable moments',
    moments: [
      'Redesigned new-hire onboarding from scratch, cutting ramp time from six weeks to two through structured training materials and standardized ticket templates.',
      'Standardized documentation for high-volume issue categories, improving team consistency and first-call resolution rates across the desk.',
      'Authored knowledge-base articles that measurably reduced repeat escalations and improved self-service resolution for common issues.',
      'Managed daily operations, scheduling, and technical escalations for a rotating team of 10–15 student technicians.',
    ],
    tags: ['Team Lead','10–15 Techs','Onboarding 6→2wk','Knowledge Base'],
  },
  {
    idx: 5,
    node: 'I', name: 'Iowa State University', elev: '1,460 ft',
    stationLabel: 'Station I · The Trailhead',
    title: 'Iowa State', titleEm: 'University',
    sub: "B.S. Cyber Security Engineering · Class of '27",
    highlights: [
      { k: 'Program',   v: null,       vEm: 'B.S.',   vSuffix: ' CSE' },
      { k: 'Since',     v: 'Fall ',    vEm: '2023',   vSuffix: null },
      { k: 'Graduates', v: 'Spring ',  vEm: '2027',   vSuffix: null },
      { k: 'Campus',    v: null,       vEm: 'Ames',   vSuffix: ', Iowa' },
    ],
    sectionHead: 'Notable moments',
    moments: [
      'Built a hardware-scheduled five-stage pipelined RISC-V processor at the assembly level, connecting software instruction execution to hardware datapath design.',
      'Deployed enterprise LDAP, DNS, web, mail, and security servers in system hardening labs.',
      'Completed penetration testing coursework and networked systems projects in controlled environments.',
      'Coursework: cryptography, computer architecture, data structures, embedded systems, networking protocols, software project management, cybersecurity fundamentals.',
    ],
    tags: ['Cryptography','RISC-V','Pen Testing','Networking','Embedded Systems'],
  },
  {
    idx: 6,
    node: 'H', name: 'The Homelab', elev: '380 ft',
    stationLabel: 'Below the Trail · The Homelab',
    title: 'The', titleEm: 'Homelab',
    sub: 'Self-Hosted Infrastructure · 2023 — Present',
    hobbyCards: [
      { icon: '🗄️', t: 'Storage & NAS', d: 'Centralized file storage with RAID, network access, and automated backups. Hands-on practice with storage management, data organization, and uptime.' },
      { icon: '🌐', t: 'Network & Pi-hole', d: 'DNS-level filtering with Pi-hole for network-wide ad blocking and traffic visibility. Covers DNS, monitoring, and local network configuration.' },
      { icon: '🤖', t: 'Local AI & Compute', d: 'Raspberry Pi 5 running Ollama with local LLMs. Offline AI experiments and self-hosted automation without cloud dependency.' },
    ],
    tags: ['Linux','NAS','Pi-hole','DNS','Raspberry Pi','Ollama','Self-Hosted','Monitoring'],
  },
  {
    idx: 7,
    node: '🌿', name: 'Off the Trail', elev: '800 ft',
    stationLabel: 'Off the Trail',
    title: 'Life', titleEm: 'beyond the work',
    sub: 'Hiking · Travel · Interests · 2023 — Present',
    photos: [
      { src: '/photos/meHiking.jpg',             label: 'On the trail' },
      { src: '/photos/hiking1.jpeg',             label: 'Hiking' },
      { src: '/photos/hiking2.jpeg',             label: 'Summit' },
      { src: '/photos/Travel%201_Mexico.jpeg',   label: 'Mexico' },
      { src: '/photos/travel2.jpeg',             label: 'Travel' },
      { src: '/photos/travelHK.jpeg',            label: 'Hong Kong' },
      { src: '/photos/Hobby%201.jpeg',           label: 'Hobbies' },
      { src: '/photos/hobby3.jpeg',              label: 'TCG' },
      { src: '/photos/hobby1b.jpeg',             label: 'Gaming' },
    ],
    hobbyCards: [
      { icon: '🏔️', t: 'Hiking & Outdoors', d: 'Logging miles on Iowa trails and beyond. Prefers ridgelines to parking lots.' },
      { icon: '✈️', t: 'Travel',             d: 'Mexico, Hong Kong, and wherever else fits a long weekend. Perpetual packing-light student.' },
      { icon: '🃏', t: 'TCG & Gaming',       d: 'Pokémon, Magic: The Gathering. Nerd Market started here — treating collections like portfolios.' },
    ],
    tags: ['Hiking','Travel','TCG','Photography','Outdoor Adventures'],
  },
];

export const NODE_POS = [
  { x: 160, y: 82  },
  { x: 173, y: 200 },
  { x: 177, y: 256 },
  { x: 165, y: 310 },
  { x: 155, y: 368 },
  { x: 116, y: 440 },
  { x: 107, y: 488 },
  { x: 100, y: 530 },
];
